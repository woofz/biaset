...
from gestionesquadra.exceptions import NumeroSquadreMassimo
...
def check_team_belonging(function=None):
    ...
    def wrapper_func(request, *args, **kwargs):
        ...
        requester_id = request.GET.get('requester_id', request.user.id)
        user = User.objects.get(pk=requester_id)
        squadra = Squadra.objects.get(pk=pk)
        campionato = Campionato.objects.filter(championship_admin=user).first()  # Controllo se è un CA
        if user.profilo.first().nome != 'Championship Admin':
            if int(squadra.allenatore.id) != int(user.id):
                messages.error(request, "Non hai i permessi per effettuare questa operazione.")
                return redirect('dashboard_index')
        return function(request, *args, **kwargs)
...
def check_partecipants(function=None):
    ...
    def wrapper_func(request, *args, **kwargs):
        ...
        if numeroSquadre == campionato.partecipanti:
            raise NumeroSquadreMassimo('Il campionato ha raggiunto già il massimo numero di squadre partecipanti.')
        return function(request, *args, **kwargs)
