    ...
    def generaCalendario(self, campionato: Campionato) -> None:
        ...
        for i in range(0, len(scheduling)):
            ...
            for squadra in scheduling[i]:
                ...
                Formazione.objects.create(tipo='T', partita=partita, squadra=squadra[0])
                Formazione.objects.create(tipo='T', partita=partita, squadra=squadra[1])
                Formazione.objects.create(tipo='R', partita=partita, squadra=squadra[0])
                Formazione.objects.create(tipo='R', partita=partita, squadra=squadra[1])
